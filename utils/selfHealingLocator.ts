...
export type SelectorFallback = string | ((page: Page) => Locator);
 ...
 * Always targets .first() and checks visibility to prevent Playwright strict mode violations.
 */
export async function locateInputWithHealing(
  page: Page,
  labelText: string,
  fallbackSelectors: SelectorFallback[] = [],
  ...
  // Pass 1: Look for currently VISIBLE elements first (crucial for responsive DOM duplicates)
...
/**
 * Fast and resilient input helper for Desktop and Mobile browsers.
...
export async function fastInputWithHealing(
  ...
  try {
    ...
    // Ultra-fast JS evaluate fallback for tricky DOM or animation states
...
export async function locateElementWithHealing(
  ...
  const rawStrategies: (() => Locator)[] = [
    ...
    () => page.getByRole('link', { name: new RegExp(labelText, 'i') }),
