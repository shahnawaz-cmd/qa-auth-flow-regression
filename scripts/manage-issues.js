...
async function manageGitHubIssues() {
  ...
  const repoFull = process.env.GITHUB_REPO; // e.g. "shahnawaz-cmd/global-authflow"
  ...
  const resultFiles = ['results.json', 'playwright-report/results.json'];
  if (fs.existsSync('playwright-reports-merged')) {
    try {
      const folders = fs.readdirSync('playwright-reports-merged');
      for (const f of folders) {
        const jsonP = path.join('playwright-reports-merged', f, 'results.json');
        if (fs.existsSync(jsonP)) resultFiles.push(jsonP);
      }
    } catch (e) {}
  }
      ...
      allSpecs.forEach((spec) => {
        ...
        for (const testInst of spec.tests) {
          ...
          if (hasPass || isSkipped) {
            ...
            failedSpecs.push({ title: spec.title, browser: testInst.projectName || 'default', error: errorMsg });
          }
        }
      });
  ...
  const runUrl = `${process.env.GITHUB_SERVER || 'https://github.com'}/${process.env.GITHUB_REPO}/actions/runs/${process.env.GITHUB_RUN}`;
  ...
  if (failedCount > 0) {
    ...
    const issueTitle = `🔴 [Automated] Global Auth Flow Production Monitoring Test Failure`;
...
---
*This issue was automatically created by the Global Auth Flow Production Stability Monitor.*`;
    ...
    if (openIssues.length > 0) {
      for (const issue of openIssues) {
        ...
        await octokit.rest.issues.createComment({
          ...
          body: `✅ **Resolved:** All Global Auth Flow tests passed cleanly in workflow run [${process.env.GITHUB_RUN}](${runUrl}). Auto-closing issue.`
        });
